...
export class CommissionsController extends CommissionsServices {
  ...
  async getAllUsersCommissionsById(req: Request, res: Response) {
    const { id } = req.params;
    try {
      const commission = await this.getUsersCommissionById(Number(id));

      if (!commission) return res.status(404).json({ status: false, msg: "Commission not found" });
