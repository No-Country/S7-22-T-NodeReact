...
export class ClassesRoutes extends BaseRouter<ClassesController, ClassesMiddlewares> {
  ...
  routes(path: string): void {
    ...
    // POST - ENPOINTS
    this.router.post(`${path}/post`, (req, res) => this.controller.createClass(req, res));

    // PUT - ENPOINTS
    this.router.put(`${path}/put/:id`, (req, res) => this.controller.updateClass(req, res));

    // DELETE - ENPOINTS
    this.router.delete(`${path}/delete/:id`, (req, res) => this.controller.deleteClass(req, res));
  }
}
